...
EcalTPGAnalyzer4MC::EcalTPGAnalyzer4MC(const edm::ParameterSet&  iConfig)
{
  ...
  vtxLabel_      = consumes<vector<reco::Vertex>>     (iConfig.getParameter<InputTag>("VtxLabel"));
  ...
  //addpu 4MC for bunch intime
  tree_->Branch("nVtx",&treeVariables_.nVtx,"nVtx/i");
  tree_->Branch("puTrue",&treeVariables_.puTrue,"puTrue/f");
...
EcalTPGAnalyzer4MC::~EcalTPGAnalyzer4MC()
{
  file_->cd();
  tree_->Write();
  treeShape_->Write();
...
void EcalTPGAnalyzer4MC::beginRun( const edm::EventSetup & iSetup,const edm::Run & iRun)
{
  myevt = 0;
...
void EcalTPGAnalyzer4MC::analyze(const edm::Event& iEvent, const  edm::EventSetup & iSetup)
{
  ...
  treeVariables_.nVtx = -1;
  if (vtxHandle.isValid()) {
    treeVariables_.nVtx= 0;
    for (vector<reco::Vertex>::const_iterator v = vtxHandle->begin(); v != vtxHandle->end(); ++v) {
      // if (v->ndof() > 4. && fabs(v->z()) <= 24. && fabs(v->position().rho()) <= 2.) can be used for good vtx
      treeVariables_.nVtx++;
    }
  }
  ...
  if (genPileupHandle.isValid()) {
    for (vector<PileupSummaryInfo>::const_iterator pu = genPileupHandle->begin(); pu != genPileupHandle->end(); ++pu) {
      ...
      treeVariables_.puTrue=pu->getTrueNumInteractions();
      }
...
DEFINE_FWK_MODULE(EcalTPGAnalyzer4MC);
